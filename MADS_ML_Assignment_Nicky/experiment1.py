...
def trainModel(trainer, model, hI, fI):
    mlflow.set_tracking_uri("sqlite:///mlflow.db")
    mlflow.set_experiment("experiment 1: hidden_filter_rel")
    ...
    with mlflow.start_run():
        ...
        mlflow.set_tag("filter_maps", fI)
        mlflow.set_tag("hidden_size", hI)
...
def main(hI, fI):
    ...
    model = LstmModel(hI,fI)
    trainer = getTrainer(settings, trainstreamer, validstreamer, model)
    
    trainModel(trainer, model, hI, fI)

    
if __name__ == "__main__":
    for hI in (2**p for p in range(10, 11)):
        for fI in (2**p for p in range(1,  11)):
            main(hI, fI)